...
class TaskCard(db.Model):
    ...
    card_id = db.Column(String(36))
    title = db.Column(String(32))
    tasks = db.relationship('Task', backref='task_card', cascade="all,delete")

class Task(db.Model):
    __tablename__ = 'Task'
    id = db.Column(Integer, primary_key=True)
    task_id = db.Column(String(36))
    content = db.Column(String(32))
    status = db.Column(db.Integer, nullable=False, default=0)
    task_card_id = db.Column(Integer, db.ForeignKey('TaskCard.id'))
